

// Scroll Reveal

ScrollReveal({
    distance: '80px',
    duration: 2000,
    delay: 200,
});

ScrollReveal().reveal('.theme-toggle, .titulo, .header, .c, .tres, .avatar, .scroll-down', {origin:'top' });
ScrollReveal().reveal('.portfolio, .social-icons, .card,  .service-card, .footer, .redes-sociais, .tiktok-button, .user-card, .quatro, .btn-whatsapp, .apoio-container, .event-list', {origin:'bottom' });
ScrollReveal().reveal('.heading, .gradient-name .padrao, .portfolio-caixa, .um, .scroll-down', {origin:'left' });
ScrollReveal().reveal('.b, .dois, .description', {origin:'right' });






